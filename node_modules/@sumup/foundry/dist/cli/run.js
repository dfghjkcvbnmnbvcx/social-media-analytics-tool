...
async function resolveTo(path, name) {
    ...
    const packageJsonPath = (0, path_1.join)(path, name);
    try {
        await accessAsync(packageJsonPath);
        return packageJsonPath;
    }
    catch (err) {
        const parentPath = (0, path_1.resolve)(path, '..');
...
async function getPackageJsonPath(name, useRelative = false) {
    ...
    return useRelative ? (0, path_1.relative)(__dirname, pathPackage) : pathPackage;
}
...
async function resolveBinaryPath(name, useRelative = false) {
    try {
        ...
        const binaryPath = (0, fp_1.isString)(packageBin) ? packageBin : packageBin[name];
        if (!binaryPath) {
            return null;
        }
        return (0, path_1.resolve)((0, path_1.dirname)(packageJsonPath), binaryPath);
    }
    catch (err) {
        return null;
    }
...
async function executeBinary(path, args) {
    return (0, spawn_1.spawn)(path, args, {
        stdio: 'inherit',
    });
}
